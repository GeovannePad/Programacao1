...
namespace ConsoleApp.Aula5
{
    public class DapperContext
    {
        ...
        public async Task<List<LivroBaixaPontuacao>> ListaTop3LivrosPoucoAvaliadosEReviews()
        {
            try
            {
                using (var dbConnection = new MySqlConnector.MySqlConnection(stringDeConexao))
                {
                    ...
                    var resultado = await dbConnection.QueryAsync<LivroBaixaPontuacao>(query);
            ...
            {
                Console.WriteLine("Erro ao consultar os dados no BD. Erro: " + ex.Message);
                return new List<LivroBaixaPontuacao>();
            }
        }

        public async Task<List<LivroPontuacaoPorAno>> ListaLivrosPorAnoEPontuacao()
        {
            try
            {
                using (var dbConnection = new MySqlConnector.MySqlConnection(stringDeConexao))
                {
                    ...
                    var resultado = await dbConnection.QueryAsync<LivroPontuacaoPorAno>(query);
            ...
            {
                Console.WriteLine("Erro ao consultar os dados no BD. Erro: " + ex.Message);
                return new List<LivroPontuacaoPorAno>();
            }
        }
    }
}
